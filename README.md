...
そこで、Salesforce専用のCI/CDツールとして無料で利用できるDevOpsCenterでCICDには、以下の利点があることがわかった。
...
- [Flosum](https://devops.terrasky.co.jp/flosum)
  - 有料のAppExchangeであるため、検証する前にDevOpsCenterを検証したほうがいいと考えた
...
※ブランチ名が正しければgit側からも変更をpushできる。リリースする資材はGithubを参照するため実現できる
...
- レビューで、昇格準備完了ができるのは、「DevOpsCenter」
- 現状、作業項目は、一度作成すると削除してUI上から無くすことができない。しかし、Github上では作成された作業ブランチはクローズされる
...
- 正しいブランチ名でPRを手動で作成しても、DevOpsCenterコンソールの「昇格準備完了」のタブは活性化する。つまり、PRはDevOpsCenterからでも手動でもどちらから作っても構わない
- パイプラインについて
...
- パイプラインの操作、上流環境への変更内容の昇格作業、つまりリリース作業は、**本番環境**での作業となる
  ...
  - DevOpsCenterをインストールすると、５種類の権限セットが自動で用意される
- レポジトリは現状GithubかBitBucketのみ
